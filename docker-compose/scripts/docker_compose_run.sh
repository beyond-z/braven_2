...
EOPHP
    ...
    echo "### Injecting ENV var supplied values into wp-config.php"
		...
		set_config 'DB_INTERVIEW_MATCHER_NAME' "$DB_INTERVIEW_MATCHER_NAME"
...
fwrite($stderr, "\n### Attempting to create the Wordpress database if they don't exist");
...
// by default using the docker image.
$user = 'root';
$wordpressUser = getenv('WORDPRESS_DB_USER');
$pass = getenv('MYSQL_ROOT_PASSWORD');
$dbName = getenv('WORDPRESS_DB_NAME');
$mockIvDbName = getenv('DB_INTERVIEW_MATCHER_NAME');
...
// TODO: This doesn't seem to be working for some reason. Just go manually run:
// docker-compose exec bravendb mysql -h bravendb -P 3306 -u root -ppass
// GRANT ALL PRIVILEGES ON `braven_interview_matcher`.* TO 'wordpress'@'%';
// CREATE DATABASE IF NOT EXISTS `braven_interview_matcher`;

if (!$mysql->query('GRANT ALL PRIVILEGES ON `' . $mysql->real_escape_string($mockIvDbName) . '`.* TO \'' . $mysql->real_escape_string($wordpressUser) . '\'@\'%\'')) {
	fwrite($stderr, "\n" . 'MySQL "GRANT ALL PRIVILEGES" Error: ' . $mysql->error . "\n");
	$mysql->close();
	exit(1);
}

if (!$mysql->query('CREATE DATABASE IF NOT EXISTS `' . $mysql->real_escape_string($mockIvDbName) . '`')) {
	fwrite($stderr, "\n" . 'MySQL "CREATE DATABASE" Error: ' . $mysql->error . "\n");
	$mysql->close();
	exit(1);
}$mysql->close();
