...
if aws --version 2> /dev/null; then
  ...
  aws s3 sync 's3://bebraven-staging-wp-content/uploads/' './wp-content/uploads/' \
    || { echo >&2 "Error: Failed pulling uploads folder from bebraven-staging-wp-content S3 bucket"; exit 1; }

  aws s3 sync 's3://bebraven-staging-wp-content/plugins/' './wp-content/plugins/' \
    || { echo >&2 "Error: Failed pulling plugins folder from bebraven-staging-wp-content S3 bucket"; exit 1; }
