		...
		<main id="main" class="site-main" role="main">
			...
			if( 'press' == $post->post_name ) {
				...
				if ( $post->post_type == 'page' && $post->post_parent == 0 && !is_home() && !is_front_page() && !is_archive() ) {
					...
					?>
					<!-- orphan page -->
					<section class="component default orphan-page">
						<?php echo apply_filters('the_content', $post->post_content); ?>
					</section>
					<?php
				} else {
					echo '<!--tabs for kids -->';
					// If it's not the blog page or some random page like a job posting, but it still has some data:
